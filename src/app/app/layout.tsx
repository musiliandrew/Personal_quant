...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative min-h-screen flex flex-col md:flex-row" style={{ background: "var(--gradient-hero)" }}>
      ...
      {/* Main Content Area */}
      <main className="flex-1 min-w-0 flex flex-col h-dvh md:h-screen overflow-hidden">
        {/* Mobile Sticky Header (Shows on every page including Quant) */}
        <div 
          className="sticky top-0 z-30 backdrop-blur-md pt-7 pb-3.5 px-4 border-b border-foreground/[0.04] mb-1 md:hidden shrink-0"
          style={{ backgroundColor: "color-mix(in oklab, var(--background) 35%, transparent)" }}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <Logo size={26} className="text-foreground shrink-0" />
              <div>
                <p className="text-[10.5px] sm:text-[12px] text-muted-foreground font-semibold">{greeting}</p>
                <div className="flex items-center gap-2">
                  <h1 className="text-[20px] sm:text-[24px] font-bold tracking-tight">{userName}</h1>
                  {isPro !== null && (
                    <span className={cn(
                      "text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0 mt-0.5",
                      isPro 
                        ? "bg-purple-500/10 text-purple-400 border border-purple-500/20" 
                        : "bg-foreground/10 text-muted-foreground"
                    )}>
        ...
        <div className={cn(
          "w-full",
          pathname === "/app/quant"
            ? "w-full flex-1 min-h-0 p-0 m-0 max-w-none flex flex-col overflow-hidden"
            : "mx-auto max-w-md md:max-w-4xl lg:max-w-5xl px-4 py-4 md:px-8 md:py-10 pb-32 md:pb-16 overflow-y-auto"
        )}>
          {/* Desktop header greeting */}
          <div className="hidden md:flex flex-col gap-0.5 mb-6">
